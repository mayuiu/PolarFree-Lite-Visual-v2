...
def showcase_visual_polish(
    ...
    hard_takeover = torch.clamp(
        ...
        + 0.20 * hard3
        + 0.24 * lowfreq3
        + 0.14 * line_text_region,
        0.0,
        0.72,
    )
    hard_takeover = torch.clamp(hard_takeover * torch.clamp(1.0 - 0.18 * guard3, 0.50, 1.0), 0.0, 0.72)
    ...
    if torch.max(lowfreq_region).detach().cpu().item() > EPS:
        ...
        low_alpha = torch.clamp(
            ...
            + 0.26 * dark_shadow_confidence
            + 0.14 * shadow_veil_mask
            + 0.10 * v2_reflection_takeover,
            0.0,
            0.58,
        )
        low_alpha = torch.clamp(low_alpha * torch.clamp(1.0 - 0.25 * effective_guard, 0.55, 1.0), 0.0, 0.58)
    ...
    tint_region = torch.clamp(
        0.45 * tint_confidence
        + 0.25 * v2_reflection_takeover
        + 0.20 * lowfreq_confidence
        + 0.20 * shadow_veil_mask,
        0.0,
        1.0,
    )
    ...
    smooth_alpha = torch.clamp(float(smooth_strength) * smooth_region, 0.0, 0.48)
...
def compose_prediction(
    ...
    blend_gate = torch.clamp(
        blend_gate * (1.0 - 0.18 * effective_guard)
        + 0.36 * safe_release
        + 0.14 * v2_reflection_takeover,
        0.06,
        0.985 if showcase_mode else 0.94,
    )
    ...
    structure_safe = torch.clamp(1.0 - 0.35 * effective_guard, 0.45, 1.0)
    ...
    direct_pull = torch.clamp(
        ...
        + 0.16 * text_force
        + 0.18 * line_force
        + 0.36 * v2_reflection_takeover
        - 0.10 * effective_guard,
        0.0,
        0.92,
    )
    if loss_profile == "visual":
        visual_pull = torch.clamp(0.38 * core + 0.18 * boundary + 0.24 * v2_reflection_takeover, 0.0, 0.90)
    ...
    if reference_rgb is not None:
        lowfreq_alpha = torch.clamp(
            0.22 * lowfreq_reflection + 0.28 * lowfreq_reflection_confidence + 0.16 * v2_reflection_takeover,
            0.0,
            0.62,
        )
        lowfreq_alpha = torch.clamp(lowfreq_alpha * torch.clamp(1.0 - 0.22 * effective_guard, 0.55, 1.0), 0.0, 0.62)
        ...
        dark_alpha = torch.clamp((0.18 * dark_reflection + 0.36 * dark_shadow_confidence + 0.16 * v2_reflection_takeover) * torch.clamp(1.0 - 0.18 * effective_guard, 0.55, 1.0), 0.0, 0.68)
        dark_delta = torch.clamp(signed_clean_delta, -0.35, 0.35)
        prediction = torch.clamp(
            prediction * (1.0 - dark_alpha.expand_as(prediction))
            + torch.clamp(prediction + 0.78 * dark_delta, 0.0, 1.0) * dark_alpha.expand_as(prediction),
            0.0,
            1.0,
        )
    ...
    if showcase_mode:
        ...
        force_direct = torch.clamp(
            ...
            + 0.22 * text_force
            + 0.55 * v2_reflection_takeover
            + 0.22 * line_band_confidence
            + 0.22 * text_reflection_confidence
            + 0.28 * lowfreq_reflection_confidence
            + 0.30 * dark_shadow_confidence
            - 0.18 * effective_guard,
            0.0,
            0.995,
        )
    else:
    ...
    if showcase_mode:
        visual_extra = torch.clamp(
            0.35 * v2_reflection_takeover
            + 0.25 * lowfreq_reflection_confidence
            + 0.25 * dark_shadow_confidence
            + 0.18 * text_reflection_confidence
            + 0.18 * line_band_confidence,
            0.0,
            0.75,
        )
        visual_extra = torch.clamp(visual_extra * torch.clamp(1.0 - 0.14 * effective_guard, 0.62, 1.0), 0.0, 0.75)
        ...
        lowfreq_mix = torch.clamp(
            0.18 * lowfreq_reflection_confidence
            + 0.18 * dark_shadow_confidence
            + 0.12 * scene_reflection
            + 0.12 * shadow_veil,
            0.0,
            0.45,
        )
        lowfreq_mix = torch.clamp(lowfreq_mix * torch.clamp(1.0 - 0.20 * effective_guard, 0.55, 1.0), 0.0, 0.45)
        ...
        weights.update(
            {
                ...
                "visualplus_should_differ_from_raw_in_reflection_region": 0.14,
