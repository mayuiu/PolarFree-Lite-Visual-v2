...
def showcase_visual_polish(
    ...
    lowfreq_confidence = torch.clamp(lowfreq_confidence if lowfreq_confidence is not None else torch.zeros_like(support_soft), 0.0, 1.0)
    dark_shadow_confidence = torch.clamp(dark_shadow_confidence if dark_shadow_confidence is not None else torch.zeros_like(support_soft), 0.0, 1.0)
    ...
    soft3 = support_soft.expand_as(showcase_result)
    hard3 = support_hard.expand_as(showcase_result)
    shadow3 = torch.maximum(lowfreq_confidence, dark_shadow_confidence).expand_as(showcase_result)
    guard3 = foreground_structure_guard.expand_as(showcase_result)
    hard_takeover = torch.clamp(0.28 * hard3 + 0.20 * shadow3 * torch.clamp(1.0 - 0.45 * guard3, 0.25, 1.0), 0.0, 0.46)
    showcase_result = torch.clamp(showcase_result * (1.0 - hard_takeover) + direct_clean * hard_takeover, 0.0, 1.0)

    if torch.max(lowfreq_confidence).detach().cpu().item() > EPS:
        low_showcase = gaussian_blur_tensor(showcase_result, sigma=4.0)
        low_direct = gaussian_blur_tensor(direct_clean, sigma=4.0)
        low_alpha = torch.clamp(0.42 * lowfreq_confidence * torch.clamp(1.0 - 0.55 * foreground_structure_guard, 0.20, 1.0), 0.0, 0.42)
        showcase_result = torch.clamp(showcase_result + low_alpha.expand_as(showcase_result) * (low_direct - low_showcase), 0.0, 1.0)

    tint_mask = torch.clamp(torch.maximum((support_soft - 0.20) / 0.55, tint_confidence), 0.0, 1.0)
    ...
    showcase_result[:, 1:2] = showcase_result[:, 1:2] - float(tint_suppress) * g_excess * tint_mask
    showcase_result[:, 0:1] = showcase_result[:, 0:1] - 0.45 * float(tint_suppress) * r_excess * tint_mask
    ...
    safe_detail_mask = torch.clamp(1.0 - torch.maximum(support_soft, 0.85 * foreground_structure_guard), 0.0, 1.0).expand_as(showcase_result)
    ...
    smooth_alpha = torch.clamp(float(smooth_strength) * hard3, 0.0, 0.45)
...
def compose_prediction(
    ...
    shadow_support = torch.clamp(torch.maximum(scene_reflection, torch.maximum(dark_reflection, torch.maximum(shadow_veil, lowfreq_reflection))), 0.0, 1.0)
    ...
    if showcase_mode:
        no_prior_zone = torch.clamp(torch.maximum(support_hard, high_confidence_reflection), 0.0, 1.0)
    ...
    prior_release = torch.clamp((high_confidence_reflection - 0.16) / 0.46, 0.0, 1.0)
    ...
    structure_safe = torch.clamp(1.0 - 0.62 * foreground_guard, 0.18, 1.0)
    ...
    text_force = torch.clamp(torch.maximum(text_reflection_confidence, 0.55 * shadow_veil), 0.0, 1.0)
    line_force = torch.clamp(line_band_confidence, 0.0, 1.0)
    tint_force = torch.clamp(tint_confidence, 0.0, 1.0)
    direct_pull = torch.clamp(
        0.20 * support_soft
        + 0.38 * high_confidence_reflection
        + 0.14 * residual_confidence
        + 0.22 * lowfreq_force
        + 0.28 * dark_force
        + 0.16 * text_force
        + 0.18 * line_force,
    ...
    if loss_profile == "visual":
        visual_pull = torch.clamp(0.38 * core + 0.18 * boundary, 0.0, 0.82)
    ...
    if reference_rgb is not None:
        lowfreq_alpha = torch.clamp((0.22 * lowfreq_reflection + 0.28 * lowfreq_reflection_confidence) * structure_safe, 0.0, 0.55)
        ...
        dark_alpha = torch.clamp((0.18 * dark_reflection + 0.36 * dark_shadow_confidence) * structure_safe, 0.0, 0.62)
        dark_delta = torch.clamp(signed_clean_delta, -0.35, 0.35)
        prediction = torch.clamp(
            prediction * (1.0 - dark_alpha.expand_as(prediction))
            + torch.clamp(prediction + 0.70 * dark_delta, 0.0, 1.0) * dark_alpha.expand_as(prediction),
            0.0,
            1.0,
        )
    glare = torch.clamp(glare_mask if glare_mask is not None else torch.zeros_like(final_mask), 0.0, 1.0)
    if showcase_mode:
        ...
        force_direct = torch.clamp(force_direct * torch.clamp(1.0 - 0.34 * foreground_guard, 0.30, 1.0), 0.0, 0.995)
    else:
        force_direct = torch.clamp(
            0.34 * support_soft + 0.50 * core + 0.18 * boundary + 0.14 * lowfreq_force + 0.18 * dark_force,
            0.0,
            0.92,
        )
...
def compute_total_loss(
    ...
    if base_prior is not None:
        ...
        visualplus_should_differ_from_raw_in_reflection_region = weighted_l1(
            torch.relu(0.018 - raw_distance).expand_as(prediction),
            torch.zeros_like(prediction),
            reflection_region,
        )
    else:
        ...
        weights.update(
            {
                ...
                "visualplus_should_differ_from_raw_in_reflection_region": 0.08,
